...
dir = "/home/amandyam/contrastive-rl/contrastive-rl/irl/clinical/gym-sepsis/gym_sepsis/envs/"
STATE_MODEL = dir + "model/sepsis_states.model"
TERMINATION_MODEL = dir + "model/sepsis_termination.model"
OUTCOME_MODEL = dir + "model/sepsis_outcome.model"
STARTING_STATES_VALUES = dir + "model/sepsis_starting_states.npz"
...
class SepsisEnv(gym.Env):
	...
	def __init__(self, reward_params, dim, starting_state=None, verbose=False):
		...
		self.params = reward_params # A vector of length 3, these only modify the sofa scores. (or 6)
		self.dim = dim
		if self.dim == 3:
			self.sofa_elts = ['sofa', 'qsofa', 'qsofa_sysbp_score']
		else:
			self.sofa_elts = ['sofa', 'LACTATE']
	...
	def step(self, action):
		...
		# Reward is the dot product of the reward params and the differences between sofa scores in this time step and the next time step
		sofa_vals = [self.s[features.index(el)][0] for el in self.sofa_elts]
		sofa_vals = [x.item() for x in sofa_vals]
		sofa_vals_ns = [next_state.flatten()[features.index(el)] for el in self.sofa_elts]

		diff = np.subtract(sofa_vals, sofa_vals_ns)
		reward = np.dot(self.params, diff)
		done = False
		...
		self.s_abbr = [self.s[features.index(el)][0] for el in self.sofa_elts]
		self.state_idx += 1
